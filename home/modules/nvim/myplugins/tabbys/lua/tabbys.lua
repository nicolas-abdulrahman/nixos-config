...
function M.setup(user_opts)
	...
	if opts.keymaps.enable then
		...
		vim.keymap.set({ "n", "t" }, p .. k.tabprev .. ">", M.tab_prev, {
			desc = "Tabby: Toggle feature",
		})
		vim.keymap.set({ "n", "t" }, p .. k.tabnext .. ">", M.tab_next, {
			desc = "Tabby: Toggle feature",
		})
		vim.keymap.set("n", p .. k.tabclone .. ">", M.tab_clone, {
			desc = "Tabby: Toggle feature",
		})
		vim.keymap.set("n", p .. k.tabonly .. ">", function()
			vim.cmd("tabonly")
		end, { desc = "Close all other tabs" })
		vim.keymap.set("n", p .. k.tabquit .. ">", function()
			vim.cmd("tabclose")
		end, { desc = "Close all other tabs" })
		for i = 0, 9 do
